  ...
  do {
    ...
    if (fgets(node->task, sizeof(node->task), stdin) == NULL) {
      ...
      if (add_task_function) {
        /* No caso da função add_task também precisamos de dar free ao node que
...
static int user_input_number(ToDo **head, bool remove_task_function)
{
  ...
  char buffer[10];
  do {
    /* Se não for a primeira vez no loop devemos limpar os restantes caracteres
     * que foram inseridos a mais no stdin */
    if (!first_time) {
      int discard;
      while ((discard = getchar()) != '\n' && discard != EOF) {
  ...
  /* Usamos o endptr para verificar onde a função strtol parou, se ela não parar
   * em '\n' significa que não chegou até ao fim da string e retornamos 0, que é
   * um número inválido */
  if (*endptr != '\n') {
    return 0;
  }
...
Não tens um save file ou não foi possível aceder às tarefas");
  ...
  while (fgets(buffer, sizeof(buffer), save_file) != NULL) {
    ...
    if (aux == NULL) {
      creat_node(&aux, head);
      /* O limite de 198 caracteres certifica que nunca vamos modificar memória
       * que não devemos */
      sscanf(buffer, "%198[^\n]", aux->task);
      /* Mesmo depois do strcat o numéro máximo de caracteres possíveis é 199
       * mais null char (200 bytes) */
      strcat(aux->task, "\n");
      aux->task_number = 1;
      aux->next_task = NULL;
      *head = aux;
